CREATE TABLE test.PasswordSalts
...
CREATE TABLE test.Users
 (UserID            INT			   NOT NULL IDENTITY PRIMARY KEY,
  ...
  SaltID            INT            FOREIGN KEY REFERENCES
							       EFR.PasswordSalts(SaltID)
 );

Create Table test.CharityUsers
(UserID	INT	NOT NULL FOREIGN KEY REFERENCES test.Users(UserID)
CharityID	INT	NOT NULL FOREIGN KEY REFERENCES test.Charities(CharityID)
PRIMARY KEY(UserID, CharityID);
);

Create Table test.Charities
(CharityID	INT	NOT NULL PRIMARY KEY
CharityName	NVARCHAR(100) NOT NULL
CharityLink	NVARCHAR(100) NOT NULL
);

CREATE TABLE test.PasswordRecovery
...
CREATE TABLE test.AchievementList
 (AchievementID     INT            FOREIGN KEY REFERENCES
								   EFR.Achievement(AchievementID),
  UserID            INT            FOREIGN KEY REFERENCES
								   EFR.Users(UserID)
);
